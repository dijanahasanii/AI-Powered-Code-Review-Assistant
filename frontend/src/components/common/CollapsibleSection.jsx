...
export default function CollapsibleSection({
  ...
  /** Padding on non-collapsible headers only; collapsible rows keep full-bleed tap target like before */
  const staticHeaderPad = 'px-4 py-3.5 sm:px-5';
  const toggleBtnCls =
  ...
  const staticHeaderCls =
    staticHeaderPad +
    ' flex w-full flex-wrap items-start gap-x-3 gap-y-2 text-left sm:flex-nowrap sm:items-center';
    ...
    <>
      ...
      <div className="min-w-0 flex-1 basis-[min(100%,14rem)] sm:basis-auto">
        ...
        {summary ? (
          ...
          expandable &&
          trailingHint && <p className="mt-1 text-[12px] text-desk-subtle">{suffix}</p>
        )}
      </div>
